      ...
      (define (trace instruction)
        (if (eq? trace-flag 'on)
            (let ((label (instruction-label instruction)))
              (newline)
              (if (not (null? label))
                  (display (list label '-> (instruction-text instruction)))
      ...
      (extract-labels (cdr text)
                      (lambda (insts labels prev-label)
                        (let ((next-inst (car text)))
                          (if (symbol? next-inst)
                              (receive insts (cons (make-label-entry next-inst insts) labels) next-inst)
                              (receive (cons (make-instruction next-inst prev-label) insts) labels '())))))))
...
(define (make-instruction text label) (list text label '()))
